...
## Redbook-Specific Rules

- Binaries only cross-compile for Windows targets — never assume they can run natively on Linux
- Tracing initialization must be simple and readable top-to-bottom; avoid convoluted patterns
- Use `.and_then()` for Option composition when building tracing layers (e.g., `Some(layer()...).and_then(json)`)
- Follow the user's established pattern: `_tracing.rs` for all tracing infrastructure, with impls on foreign types grouped there
